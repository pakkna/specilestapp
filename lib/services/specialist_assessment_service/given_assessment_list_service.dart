...
class GivenAssessmentListService with ChangeNotifier {
  var givenassessmentlist;

  fetchGivenAssessmentList() async {
    ...
    var response = await http.get(
        Uri.parse('${Api().baseUrl}/doctor_note_list/$patientId'),
        // body: data,
        headers: header);
    ...
    if (response.statusCode == 200) {
      givenassessmentlist =
          GivenAssessmentListModel.fromJson(jsonDecode(response.body));

      notifyListeners();
    } else {
      //something went wrong

      givenassessmentlist = "error";
    }
  }
}
