...
import 'package:design/models/visit_records_model.dart';
import 'package:flutter/material.dart';
import 'package:http/http.dart' as http;
import 'package:shared_preferences/shared_preferences.dart';

class VisitRecordsService with ChangeNotifier {
  var visitrecords;
  ...
  visitRecords() async {
    ...
    var response = await http.get(
        Uri.parse('${Api().baseUrl}/sp_doctor_prescription_records'),
        // body: data,
        headers: header);
    ...
    if (response.statusCode == 200) {
      visitrecords = VisitRecordsModel.fromJson(jsonDecode(response.body));
      // for (int i = 0; i < defaultnamelist.data.length; i++) {
      //   diagnosisNameList.add(defaultnamelist.data[i]);
      // }
      print(visitrecords.data);
